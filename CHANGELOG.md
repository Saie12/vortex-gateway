...
- **`x-request-id` correlation**: Every response now includes an `velarium-gateway-<hex>`
...
### Fixed
- `velarium-gateway serve` now uses the `host` config field for IPv4 binding instead of
...
- `localhost_only` middleware now correctly recognizes IPv4-mapped IPv6
  loopback addresses (`::ffff:127.0.0.0/104`). This fixes CLI `velarium-gateway status`
  failures when the server is bound to a dual-stack `[::]` socket.

## [0.1.7] - 2026-06-01

### Fixed
- `velarium-gateway --version` now reports the actual crate version from Cargo.toml
  instead of a hardcoded `0.1.0` string (broken since v0.1.5).
  Uses `env!("CARGO_PKG_VERSION")` via clap derive.

### Added
- Admin CLI commands now include `velarium-gateway provider list|offline|online|reset`
...
- CLI: `velarium-gateway provider list|offline|online|reset` subcommands — manage providers without curl.
...
- XDG config path support (`~/.config/velarium-gateway/config.toml` with `./config.toml` fallback).
...
### Added
- `velarium-gateway stop` subcommand — gracefully stops the daemon via SIGTERM.
- `velarium-gateway serve -v` / `-vv` — verbosity flags for per-request routing info or full trace.
...
- Last request time per provider — shown in `/status` and `velarium-gateway status` ("Just now", "5m ago", etc.).
...
- Full `velarium-gateway status` output sample in README showing virtual model routing table.

## [0.1.4] - 2026-05-30

### Added
- Local per-provider request/success/token counters visible via `GET /status` and `velarium-gateway status` — no external collector needed.
...
### Fixed
- Calibrated CI coverage thresholds to match actual coverage (workspace 43%, velarium-gateway-core 55%, velarium-gateway 36%) — thresholds now set ~3pp below measured values so regressions are caught without false failures.
