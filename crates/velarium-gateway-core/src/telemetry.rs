...
pub enum TelemetryEvent {
    RecordTransaction {
        ...
        request_id: String,             // velarium-gateway-generated request correlation ID
    },
    UpdateStatus {
        provider: String,
...
impl TelemetryClient {
    ...
    pub fn emit(&self, event: TelemetryEvent) {
        if let Err(_e) = self.sender.try_send(event) {
            ...
            warn!(
                target: "velarium-gateway::telemetry",
...
impl TelemetryWorker {
    ...
    pub fn spawn(
        otel_endpoint: &str,
        rx: mpsc::Receiver<TelemetryEvent>,
    ) -> Result<tokio::task::JoinHandle<()>, crate::error::VelariumGatewayError> {
        // Build Resource Metadata
        let resource = Resource::builder()
            .with_service_name("velarium-gateway")
        ...
        match (span_exporter, metric_exporter) {
            (Ok(se), Ok(me)) => {
                ...
                let meter = global::meter("velarium-gateway-metrics");
    ...
    async fn run_loop(&mut self) {
        let tracer = global::tracer("velarium-gateway-tracer");
        ...
        while let Some(event) = self.rx.recv().await {
            match event {
                ...
                TelemetryEvent::RecordTransaction {
                    ...
                    let span_builder = tracer.span_builder("velarium-gateway.transaction");
