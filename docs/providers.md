...
`velarium-gateway` routes requests through a configurable pool of LLM providers. This guide documents the free-tier services supported by the default `config.toml`, what models are available, and their typical rate limits.
...
- **Monitor your rate limits** via `velarium-gateway status` — if a provider consistently returns
